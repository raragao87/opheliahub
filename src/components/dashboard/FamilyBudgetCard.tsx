...
const FamilyBudgetCard: React.FC = () => {
  const navigate = useNavigate();
  const [familyBudgets, setFamilyBudgets] = useState<(Budget & { id: string })[]>([]);
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        try {
          const budgets = await getBudgetsByCategory(user.uid, 'family');
  ...
  const activeBudgets = familyBudgets.filter(budget => budget.isActive);
  ...
  return (
    ...
    >
      ...
      <h3 className="text-xl font-semibold text-gray-900 mb-3">Family Budget</h3>
      <p className="text-gray-600 leading-relaxed mb-4">
        Create and manage monthly family budgets with tag-based categories and real-time tracking.
      </p>
      {loading ? (
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-semibold text-gray-900">{familyBudgets.length}</span>
          </div>
          <div className="flex items-center justify-between text-sm">
...
export default FamilyBudgetCard;
